...
%UPDATED TO COMPARE WITH CLEAN SIGNAL AND RUN MULTIPLE IDENT SIGNALS
...
noise_level_iters = 15;
set_output_noise_initial = 1e-16;        %Output Noise Power
...
%accuracy_validation = [];
accuracy_validation_phys = [];
accuracy_validation_prbs = [];
...
for signal = 1:signals
    ...
    for ident_trial = 1:num_ident_trials
    ...
    %         ylabel('|Pxx2(f)|')
        ...
        accuracy_identification_all(ident_trial,:,signal) = accuracy_identification;
        noise_snr_all_clean(ident_trial,:,signal) = noise_snr_clean;
        noise_snr_all(ident_trial,:,signal) = noise_snr;
        output_noise_power_all(ident_trial,:,signal) = output_noise_power;

        if signal == 1
            if LNL_model == true
                models_phys_all = [models_phys_all;LNL_all];
            elseif Hammerstein_model == true
                models_phys_all = [models_phys_all;Hammerstein_all];
            elseif Weiner_model == true
                models_phys_all = [models_phys_all;Weiner_all];
            elseif Linear_IRF_model == true
                models_phys_all = [models_phys_all;IRF_model_all];
            else
                models_phys_all = [models_phys_all;NLN_all];
            end
        else
            if LNL_model == true
                models_prbs_all = [models_prbs_all;LNL_all];
            elseif Hammerstein_model == true
                models_prbs_all = [models_prbs_all;Hammerstein_all];
            elseif Weiner_model == true
                models_prbs_all = [models_prbs_all;Weiner_all];
            elseif Linear_IRF_model == true
                models_prbs_all = [models_prbs_all;IRF_model_all];
            else
                models_prbs_all = [models_prbs_all;NLN_all];
            end 
...
%%
for val_trial = 1:num_val_trials
        ...
        for signal = 1:signals
            ...
            for ident_trial = 1:num_ident_trials
                ...
                for noise_level = 1:noise_level_iters 
                    ...
                    if signal == 1
                        figure(signal)
                        [R, V, yp] = nlid_resid(models_phys_all(ident_trial,noise_level),Zcur);

                        accuracy_validation_phys(val_trial,noise_level,ident_trial) = V;
                    else
                        figure(signal)
                        [R, V, yp] = nlid_resid(models_prbs_all(ident_trial,noise_level),Zcur);

                        accuracy_validation_prbs(val_trial,noise_level,ident_trial) = V;
                    end
            ...
            for ident_trial = 1:num_ident_trials
                ...
                for noise_level = 1:noise_level_iters 
                    ...
                    if signal == 1
                        ...
                        accuracy_validation(val_trial,noise_level,ident_trial) = V;
                    else
                        figure(signal)
                        [R, V, yp] = nlid_resid(models_prbs_all(ident_trial,noise_level),Zcur);
    ...
    elseif Linear_IRF_model == true
        ...
        for signal = 1:signals
            ...
            for noise_level = 1:noise_level_iters 
                ...
                accuracy_validation(val_trial,noise_level,signal) = V;
...
accuracy_identification_max = max(0, accuracy_identification_all);
accuracy_identification_mean = mean(accuracy_identification_max,1);
accuracy_identification_std = std(accuracy_identification_max,1);
...
figNum = figNum+1;
hold on
plot(noise_snr_all_clean(2,:),accuracy_identification_mean(:,:,2),'LineWidth',3);
plot(noise_snr_all_clean(1,:),accuracy_identification_mean(:,:,1),'LineWidth',3);
hold off
ax = gca;
ax.FontSize = 15;
title('Identification Accuracy vs Output Noise','Fontsize',24);
...
accuracy_validation_phys_max = max(0, accuracy_validation_phys);
accuracy_validation_phys_mean = mean(accuracy_validation_phys_max,1);
accuracy_validation_phys_std = std(accuracy_validation_phys_max,1);

accuracy_validation_phys_mean2 = mean(accuracy_validation_phys_mean,3);
accuracy_validation_phys_std2 = std(accuracy_validation_phys_std,3);

accuracy_validation_prbs_max = max(0, accuracy_validation_prbs);
accuracy_validation_prbs_mean = mean(accuracy_validation_prbs_max,1);
accuracy_validation_prbs_std = std(accuracy_validation_prbs_max,1);

accuracy_validation_prbs_mean2 = mean(accuracy_validation_prbs_mean,3);
accuracy_validation_prbs_std2 = std(accuracy_validation_prbs_std,3);
...
figNum = figNum+1;
hold on
plot(noise_snr_all_clean(2,:),accuracy_validation_prbs_mean2(:,:),'LineWidth',3);
plot(noise_snr_all_clean(1,:),accuracy_validation_phys_mean2(:,:),'LineWidth',3);

plot(noise_snr_all_clean(2,:),min(accuracy_validation_prbs_mean2(:,:)+accuracy_validation_prbs_std2(:,:),100),'LineStyle','none','LineWidth',2)
plot(noise_snr_all_clean(2,:),max(accuracy_validation_prbs_mean2(:,:)-accuracy_validation_prbs_std2(:,:),0),'LineStyle','none','LineWidth',2)
patch([noise_snr_all_clean(2,:) fliplr(noise_snr_all_clean(2,:))], [min(accuracy_validation_prbs_mean2(:,:)+accuracy_validation_prbs_std2(:,:),100) fliplr(max(accuracy_validation_prbs_mean2(:,:)-accuracy_validation_prbs_std2(:,:),0))], 'b','FaceAlpha',0.2)

plot(noise_snr_all_clean(1,:),min(accuracy_validation_phys_mean2(:,:)+accuracy_validation_phys_std2(:,:),100),'LineStyle','none','LineWidth',2)
plot(noise_snr_all_clean(1,:),max(accuracy_validation_phys_mean2(:,:)-accuracy_validation_phys_std2(:,:),0),'LineStyle','none','LineWidth',2)
patch([noise_snr_all_clean(1,:) fliplr(noise_snr_all_clean(1,:))], [min(accuracy_validation_phys_mean2(:,:)+accuracy_validation_phys_std2(:,:),100) fliplr(max(accuracy_validation_phys_mean2(:,:)-accuracy_validation_phys_std2(:,:),0))], 'y','FaceAlpha',0.2)
hold off
ax = gca;
ax.FontSize = 15;
title('Validation Accuracy vs Output Noise','Fontsize',24);
